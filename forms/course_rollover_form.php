...
class block_course_rollover_form extends moodleform
{
    ...
    private function build_rollover_form()
    {
        ...
        $mform->addElement('date_selector', 'scheduled_date', get_string('scheduled_date', 'block_course_rollover'));
        $mform->addHelpButton('scheduled_date', 'scheduled_date', 'block_course_rollover');
        // course is the course in sists
        if ($courses = course_rollover::get_course_list($this->_customdata['mod_code'])) {
            $mod_code = $mform->addElement('select', 'mod_code', get_string('mod_code', 'block_course_rollover'), $courses);
            $mod_code->setSelected($this->_customdata['mod_code']);
            $mform->addHelpButton('mod_code', 'mod_code', 'block_course_rollover');
        } else {
            $mod_code = $mform->addElement('text', 'mod_code', get_string('mod_code', 'block_course_rollover'), 'maxlength="100" size="20"');
            $mform->setDefault('mod_code', $this->_customdata['mod_code']);
            $mform->addHelpButton('mod_code', 'mod_code', 'block_course_rollover');
        }
